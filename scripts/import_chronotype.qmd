...
data <- 
  data |> 
  mutate(record_id = sprintf("KNUST_S%03d", record_id))
```
...
3 participants provided an unlikely early end of work - these will be increased by 12 hours

```{r}
Id_to_shift <- c("KNUST_S001", "KNUST_S009", "KNUST_S011")

data <-
  data %>% 
  mutate(across(contains(c("work_end")), 
                \(x) case_when(
                  record_id %in% Id_to_shift ~ (x + hms::hms(hours = 12)) %>% hms::as_hms(),
                  .default = x))
         )
...
chronotype <- data |> select(record_id:sjl)
```
...
```{r}
chronotype <- left_join(chronotype, data |> select(record_id, meq), by = "record_id")
chronotype <- 
...
chronotype <- chronotype |> relocate(meq, msf_sc, sjl, .after = record_id)
