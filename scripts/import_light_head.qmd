...
Apparently, `KNUST_S003`, `KNUST_S004`, and `KNUST_S005` suffer from some kind of defect, where the data become irregular and show values around 10^4 lx, even at night. We first bin the irregular data into the next regular interval, and then convert implicit to explicit gaps. We will further remove unreasonably high values above 100.000 lx melanopic EDI
...
data_cleaned <-
  ...
  group_modify( #regularize data
    \(dat,key) if(key$Id %in% c("KNUST_S003", "KNUST_S004", "KNUST_S005")) {
      dat |>  aggregate_Datetime()
    } else dat
  ) |> 
  gap_handler(full.days = TRUE) #remove implicit gaps

data_cleaned |> count_difftime() |> filter(difftime == 0) #no duplicates
